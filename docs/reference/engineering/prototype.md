...
single self-contained HTML file with free-play buttons and guided walkthroughs handles
logic/state questions; several structurally different UI variants on one route,
switchable from a floating bottom bar, handle "what should this look like" questions. No
tests, no persistence, no abstractions. The point is to learn something fast, then fold
the validated decision into the real code and park the prototype itself on a throwaway
branch as evidence.

## When to reach for it

The moment a question can't be settled by talking, like an edge case that's hard to hold
in your head or a screen that's hard to picture until a few options sit side by side.
...
No. The two branches produce structurally different artifacts, and picking wrong wastes
the whole prototype. If it's genuinely ambiguous and no one's around to ask, default to
whichever matches the surrounding code and say so at the top.

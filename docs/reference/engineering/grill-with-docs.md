...
works when aimed at a repo rather than a change, as in "help me document my repo".
Scope decides between this and `wayfinder`. Use this for anything you can settle in one
session, and `wayfinder` when the effort needs charting as decision tickets first. With
no repo at all, use `grill-me`.
...
doesn't work. It's the stateful sibling of `grill-me`; `wayfinder` sits upstream for

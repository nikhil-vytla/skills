...
SHA-pinned file tables). Also fixes naming: `[JIRA-XXX] Short description` for titles,
`<username>-YYYYMMDD-camelCaseDescription` for branches, and a trailing `JIRA:` footer.
Ships a standalone `PR_TEMPLATE.md` for repos where a checked-in
`.github/PULL_REQUEST_TEMPLATE.md` isn't an option.

## When to reach for it

Model-invoked — fires when drafting or improving a PR body, especially for stacked PR
trains in large repos. Reach for `PR_TEMPLATE.md` directly by copying it into the PR
